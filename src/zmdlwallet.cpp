...
CzMDLWallet::CzMDLWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmdl seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMDLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMDLSeed_deprecated()) {
                    LogPrintf("%s: Updated zMDL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmdl seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmdl wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMDLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMDLCount(nCountLastUsed);
    else if (!walletdb.ReadZMDLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMDLWallet::Lock()
{
    seedMaster = 0;
}

void CzMDLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMDLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMDL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMDLWallet::LoadMintPoolFromDB()
{
...
void CzMDLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMDLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMDLWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmdlTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMDLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMDL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmdlTracker which also adds to database
    pwalletMain->zmdlTracker->Add(dMint, true);
...
void CzMDLWallet::SeedToZMDL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMDLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMDLWallet::GenerateDeterministicZMDL(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMDLWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMDLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
