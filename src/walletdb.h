...
public:
    ...
    bool WriteZMDLSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMDLSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMDLSeed_deprecated(uint256& seed);
    bool EraseZMDLSeed();
    bool EraseZMDLSeed_deprecated();

    bool WriteZMDLCount(const uint32_t& nCount);
    bool ReadZMDLCount(uint32_t& nCount);
