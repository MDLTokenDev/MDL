...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "mdl.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "mdld.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 21212, 12121));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, mdl, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-mdlstake=<n>", strprintf(_("Enable or disable staking functionality for MDL inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zmdlstake=<n>", strprintf(_("Enable or disable staking functionality for zMDL inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzmdl=<n>", strprintf(_("Enable automatic wallet backups triggered after each zMDL minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zmdlbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zMDL backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizemdlamount=<n>     " + strprintf(_("Keep N mdl anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 21222, 12111));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The MDL Community Token Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MDL Community Token Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MDL Community Token Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //mdl: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //mdl specific: zerocoin and spork DB's
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MDL Community Token Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart MDL Community Token Core to complete") << "\n";
        ...
        zwalletMain = new CzMDLWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zMDLWallet
        uiInterface.InitMessage(_("Syncing zMDL wallet..."));

        bool fEnableZmdlBackups = GetBoolArg("-backupzmdl", true);
        pwalletMain->setZmdlAutoBackups(fEnableZmdlBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zmdlTracker->Init();
...
//    }
//
//    nAnonymizemdlAmount = GetArg("-anonymizemdlamount", 0);
//    if (nAnonymizemdlAmount > 999999) nAnonymizemdlAmount = 999999;
//    if (nAnonymizemdlAmount < 2) nAnonymizemdlAmount = 2;
