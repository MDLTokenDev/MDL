...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered mdl address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal mdl:URI (mdl:XyZ...?message=example)
            {
...
static void NotifyzMDLReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzMDLReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzMDLReset.connect(boost::bind(NotifyzMDLReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzMDLReset.disconnect(boost::bind(NotifyzMDLReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zmdlTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
