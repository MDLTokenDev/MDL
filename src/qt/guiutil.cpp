...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a mdl address (e.g. %1)").arg("M9ih48pbH4hyaQA2Y4b7k5CzTwKWiRVyHN"));
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert mdl:// to mdl:
    //
    //    Cannot handle this later, because mdl:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open mdl.conf with the associated application */
...
{
    return GetAutostartDir() / "mdl.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a mdl.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the mdl app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add mdl app to startup item list
