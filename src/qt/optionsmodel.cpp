...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizemdlAmount"))
        settings.setValue("nAnonymizemdlAmount", 1000);

    nAnonymizemdlAmount = settings.value("nAnonymizemdlAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizemdlAmount"))
        SoftSetArg("-anonymizemdlamount", settings.value("nAnonymizemdlAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mdl.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizemdlAmount:
            return QVariant(nAnonymizemdlAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizemdlAmount:
            nAnonymizemdlAmount = value.toInt();
            settings.setValue("nAnonymizemdlAmount", nAnonymizemdlAmount);
            emit anonymizemdlAmountChanged(nAnonymizemdlAmount);
            break;
        case CoinControlFeatures:
