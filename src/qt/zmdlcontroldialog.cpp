...
#include "ui_zmdlcontroldialog.h"
...
std::set<std::string> ZmdlControlDialog::setSelectedMints;
std::set<CMintMeta> ZmdlControlDialog::setMints;

ZmdlControlDialog::ZmdlControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZmdlControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZmdlControlDialog::~ZmdlControlDialog()
{
    delete ui;
}

void ZmdlControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZmdlControlDialog::updateList()
{
...
void ZmdlControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZmdlControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZmdl_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZmdlControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZmdlControlDialog::GetSelectedMints()
{
...
void ZmdlControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
