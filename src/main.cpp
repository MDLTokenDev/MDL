...
// Copyright (c) 2020 The MDL Community Token Developers
...
/** Fees smaller than this (in umdl) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zMDL spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zMDL is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend.HasValidSignature())
                return error("%s: V2 zMDL spend does not have a valid signature\n", __func__);
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zMDL without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zMDL spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool: zMDL spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zmdl");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zMDL mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() != CBaseChainParams::MAIN)
        return nHeight >= Params().TreasuryStartBlock() ? 95 * COIN : 100 * COIN;
    ...
    if (nHeight == 1) {
        nSubsidy = 24702400 * COIN;
    } else if (nHeight > 1 && nHeight <= 1000) {
        nSubsidy = 0 * COIN;
    } else if (nHeight > 1000 && nHeight <= 303400) {
        nSubsidy = 16.5 * COIN;
    } else if (nHeight > 303400 && nHeight <= 605800) {
        nSubsidy = 33 * COIN;
    } else if (nHeight > 605800 && nHeight <= 908200) {
        nSubsidy = 50 * COIN;
    } else if (nHeight > 908200 && nHeight <= 1210600) {
        nSubsidy = 50 * COIN;
    } else if (nHeight > 1210600 && nHeight <= 1513000) {
        nSubsidy = 50 * COIN;
    } else if (nHeight > 1513000 && nHeight <= 1815400) {
        nSubsidy = 33 * COIN;
    } else if (nHeight > 1815400 && nHeight <= 2117800) {
    	nSubsidy = 16.5 * COIN;
    } else {
    	nSubsidy = 0 * COIN;
    }
    ...
    if (nHeight >= Params().TreasuryStartBlock())
        nSubsidy = nSubsidy * 9.5 / 10; // remove 5% for treasury (95/100 goes to stakers and MNs)
...
int64_t GetMasternodePayment(int nHeight, unsigned mnlevel, int64_t blockValue)
{
    ...
    if (nHeight >= Params().TreasuryStartBlock())
        blockValue = blockValue * 10 / 9.5; // add back treasury percentage to get original block value
...
int64_t GetTreasuryAward(int nHeight)
{
    if (IsTreasuryBlock(nHeight)) {
        ...
        blockValue = blockValue * 10 / 9.5; // add back treasury payment to get original block value
        return blockValue / 5; // 5% of block value paid to treasury
    } else {
        return 0;
    }
}
...
void RecalculateZMDLMinted()
{
    ...
    uiInterface.ShowProgress(_("Recalculating minted ZMDL..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZMDL..."), percent);
        }
...
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZMDL..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZMDL..."), percent);
        }
...
bool RecalculateMDLSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating MDL supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating MDL supply..."), percent);
        }
...
bool UpdateZMDLSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //Track zMDL money supply in the block index
    if (!UpdateZMDLSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zMDL supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMDLSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zMDL spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zMDL serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zMDL stake block is less than allowed start height", __func__);

    if (CZmdlStake* zMDL = dynamic_cast<CZmdlStake*>(stake)) {
        CBlockIndex* pindexFrom = zMDL->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zMDL stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zMDL stake does not have required confirmation depth. Current height %d, stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zMDL->GetValue()));
        if (nChecksum200 != zMDL->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zMDL->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZMDL() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zMDL fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMDL mints and %d zMDL spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // mdl: We use certain sporks during IBD, so check to see if they are
