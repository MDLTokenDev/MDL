...
#ifndef mdl_ZMDLWALLET_H
#define mdl_ZMDLWALLET_H
...
public:
    CzMDLWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMDL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMDL(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //mdl_ZMDLWALLET_H
